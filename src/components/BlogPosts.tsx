export default function BlogPosts({
  ...
  return (
    <>
      ...
      <div className="hidden md:flex flex-col justify-center items-center group gap-y-[15px]  bg-white shadow-normal dark:bg-zinc-700 rounded-2xl h-72  px-2.5 pt-1.5 w-[300px] ">
        <div className="relative w-[280px] h-[187px] ">
          <img
            src={img}
            alt=""
            loading="lazy"
          ...
          <div className="w-full h-full relative flex justify-center items-center group-hover:visible group-hover:opacity-80 opacity-0 invisible transition-all delay-75 ">
            <svg className="text-amber-900 w-[138px] h-[54px] absolute">
        ...
        <div className="flex items-start justify-center ">
          <a
            href="#"
            className="text-zinc-700 dark:text-white text-lg w-[193px] line-clamp-2 "
          >
            {title}
          </a>
          <div className="flex flex-col justify-center items-center text-teal-600 dark:text-emerald-500 border-r dark:border-white/10 pr-5 mr-5">
            <span className="font-DanaMedium text-2xl/[34.32px]">{date}</span>
      ...
      {/*Blogs and Posts for Mobile and Tablet*/}
      <div className="md:hidden group  flex justify-start items-center gap-x-3 h-[150px] rounded-2xl w-full bg-white shadow-normal dark:bg-zinc-700 group px-1.5">
        <div className="relative h-[129px] w-[129px] shrink-0 overflow-y-hidden rounded-tr-2xl rounded-tl-2xl rounded-br-2xl rounded-bl-4xl">
          <img
            src={img}
            alt="blog"
            className="h-[130px] w-[219.69px] absolute rounded-tr-2xl rounded-tl-2xl rounded-br-2xl rounded-bl-4xl object-cover"
          />
          <div className="translate-y-full group-hover:translate-y-0 transition-all absolute w-full h-full bg-gradient-to-l from-orange-300 to-orange-200 rounded-tr-2xl rounded-tl-2xl rounded-br-2xl rounded-bl-4xl opacity-80 "></div>
          <div className="w-full h-full relative flex justify-center items-center invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all   ">
            <svg className="w-[84px] h-8 text-amber-900 absolute ">
              <use xlinkHref="#logo-type"></use>
            </svg>
          </div>
        </div>
        <div className="flex flex-col divide-y  divide-gray-100 dark:divide-white/10 w-full">
          <a
            href=""
            className="inline-block w-auto font-DanaMedium text-sm sm:text-lg leading-7 line-clamp-2 text-zinc-700 dark:text-white mb-[18px]"
          >
            {title}
          </a>
          <div className="flex pt-[18px] justify-between items-center ">
            <div className="flex gap-1 text-teal-600 dark:text-emerald-500 text-xs/[17.16px] sm:text-sm ">
              <span>{date}</span>
              <span>{month}</span>
              <span>{year}</span>
            </div>
            <a
              href=""
              className="flex items-center text-xs sm:text-lg justify-center font-medium gap-x-1 text-orange-300 bg-orange-200/20 rounded-md px-2.5 py-0.5 w-auto h-auto"
            >
              مطالعه
